...
extern u16 recv_data_len;
extern char recv_teds[];
...
//	return TBC_send_noArgucmd(item.alias,item.tdcn,item.cmd_class,item.cmd_func);
    printf("cmd send: \r\n alias = %d tdcn = %d recv_data_len = %d\r\n",item.alias,item.tdcn,recv_data_len);
	for (i = 0; i < recv_data_len; i ++)
	    printf("%d ", recv_teds[i]);
	printf("\r\n cmd send start\r\n");
    TBC_send_cmd(item.alias,item.tdcn,recv_teds,recv_data_len);
}
