...
//	printf("f = %f\r\n", 1234.5678);
	sem=sys_sem_new(0);
	ssnp_init(ssnp_init_done,&sem);		 // return;
...
err_t tbc_send_follow_up_msg(int count)
{
	return send_follow_up_msg(&netif, count);
}err_t tbc_send_sync_time_msg(void)
{
	return send_sync_time_msg(&netif);
}
...
err_t tbc_send_reflect_delay_to_tbim(u8t alias, unsigned int delay)
{
	return send_reflect_delay_to_tbim(&netif,alias, delay);
}
#endif
/*******************************************************************************
...
void USART2_IRQHandler(void)       //串口接收中断，并将接收到得数据发送出
{
	 ...
	 if (isRecvFinish)
	 {
	    // for (i = 0; i < recv_len; i ++)
		  ...
		  //for (i = 1; i < recv_len - 1; i ++)
		 //    printf("%c",recv_buf[i]);
		  if ((char)recv_buf[1] == 'c' && (char)recv_buf[2] == 'm' && (char)recv_buf[3] == 'd')
		  {
		  	
		  	flagMeasure = 1;
		  }	
	//	 for (i = 0; i < recv_len - 4; i ++)
	//	     recv_teds[i] = (char)recv_buf[i + 3];
		// printf("receve finished\r\n");
		// recv_data_len = recv_len - 4;
	    // for (i = 0; i <  recv_data_len; i ++)
		 //    printf("%d ", recv_teds[i]);
		//	 printf("\r\n");
	/*    */
	//	 writeTEDS();
        get_up_cmd_alias= (int)recv_buf[1];
	    get_up_cmd_tdcn= (int)recv_buf[2];
