...
#define POPULATION_SIZE 99999999900
int testCount = 0;
...
//		cout << endl;
		...
		if (addNew == tempNew) {
			tempNew = 1200 - cnt;
			tempNew = min(200, max(40, tempNew));
			addNew = 0;
			exitTime ++;
			//cout << "add New ********** " << endl;
			goto addN;
		...
		addN:		//cout << "add New population " << endl;
				int factorSize = 190; //validPopulationSize * 199 / 1; 
				...
				if (true == addNewPopulation(v, bin, p, ee, oo, factorSize )) {
					...
					for (int i = p.size() - 1; i >= 0; i --, kk -- /*kk = (kk > validPopulationSize / 2) ? kk - 2 : kk - (rand() % 2 == 0) ? 1 : 2 */) {
						population[kk] = p[i];
						e[kk] = ee[i];
						o[kk] = oo[i];
					}
