...
static void TIM5_Init()
{
	TIM_TimeBaseInitTypeDef Tim5;  
  	...
  	Tim5.TIM_Period=100-1;	//注意TBIM中断周期100us
...
//#if DEBUGING == 0
	...
	//data=get_sensor_data(tdcn_sn);//这里使用一个假定的数据
	 printf("send.\r\n");
	++number;	
	if(data != NULL)
	{
...
void TIM5_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM5,TIM_IT_Update)!= RESET )
	{
		...
		{  //	printf("send_number is %d.\r\n",send_number);
			if(send_number>=6 && send_number<=6 )  //最大可以41 24
			{
			//	a=TIM_GetCounter(TIM5);
