...
struct schedultable_item* get_schedul_table_item(void)
{
	return tds.item;
}
/*
...
unsigned int tbim_time = 0;
int *pTBIM = NULL;
unsigned int T1 = 0;   //测量延迟用
unsigned int T2 = 0;
err_t send_reflect_reply_msg(struct netif* netif)
{
	...
	*iset=Reflect_reply;
	tbim_time =TIM_GetCounter(TIM5);
//	printf("tbim_time = %d\r\n", tbim_time);
   // printf("TBIM send delay to TBC\r\n");
	iset ++;
	pTBIM = (unsigned int*) iset;
	*pTBIM = T1;
	pTBIM ++;
	*pTBIM = T2;
	pTBIM ++;
...
int offset2 = 0;
int flagOffset = 1;
extern int epochPrev = 0;
u8t flagBeginEpoch = 0;
int timeOFF[10];
...
//	printf("receive data\r\n");
	...
	if((*msg_code) < Initiate_discovery || (*msg_code) >= Max_msg)
	{   
		printf("prev is %d.\r\n",*(msg_code -4));
		printf("prev is %d.\r\n",*(msg_code -3));
		printf("prev is %d.\r\n",*(msg_code -2));
...
//*******************************************TBIM特有代码******************************************************************
	...
	{	 //printf("ethbcp_input():TBIM recv beginning of epoch msg.\r\n");
	   // timerCount = TIM_GetCounter(TIM5);;
		//epochPrev = timerCount <= 0xffff - tbimDelayTime ? timerCount + tbimDelayTime : tbimDelayTime -(0xffff - timerCount); 
	    msg_code ++;
		pCount = (u16 *) msg_code;
		epochPrev = *pCount;
	//	epochPrev = epochPrev -	 tbimDelayTime - timeOffset;
	//	printf("epochPrev = %d pCount = %d\r\n", epochPrev, TIM_GetCounter(TIM5) + timeOffset);
		flagBeginEpoch = 1;		
		break;
	}
	case Reflect:
	...
	//	if(!tds.reflect_done)
	//	{
			T2 = TIM_GetCounter(TIM5);
			msg_code ++;
			pT = (unsigned int*) msg_code;
			T1 = *pT;
	...
	case Delay_time:
	{
			...
			pT = (unsigned int *) msg_code;
			tbimDelayTime = *pT;
		//	printf("tbimDelayTime = %d \r\n", tbimDelayTime);
				
			break;
  }
    case Sync_time:
	{
	//    printf("recv sync_time msg\r\n");
		timeSync = TIM_GetCounter(TIM5);   
		break; 
	}
	case Follow_up:
	{
	 //   printf("recv follow up msg \r\n");
	    msg_code ++;
		pT = (int *) msg_code;
		timeFollowup = *pT;
		expectTime = timeFollowup > tbimDelayTime ? timeFollowup - tbimDelayTime : 0xffff - tbimDelayTime + timeFollowup;
	
		timeOffset = expectTime - timeSync;
/*		if (flagOffset == 1) {
			timeOffset = expectTime - timeSync;
			flagOffset = 2;
		}
		else {
			timeOffset = expectTime - timeSync + 15;
		}
	//	printf("timeOffset = %d\n", timeOffset);
...
/*		
		...
		else if (flagOffset == 2) {
			offset2 = timeOffset;
			flagOffset ++;
		}
		else {
		timeOffset += (offset1 - offset2) / 2;
		}
		if (timeOffset > 0) {
...
*/	//	printf("timeFollowup = %d expectTime = %d  timeOffset = %d timeSync = %d tbimDelayTime = %d\r\n", timeFollowup, expectTime, timeOffset, timeSync, tbimDelayTime); 
		break;
	}
