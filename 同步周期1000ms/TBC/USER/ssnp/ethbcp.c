...
//	*(++iset)=alias;
	iset ++;
	pT = (unsigned int *) iset;
	cur_time = TIM_GetCounter(TIM5);
...
err_t send_reflect_delay_to_tbim(struct netif* netif,u8t alias, unsigned int delay)
{
	struct ethbcp_hdr* bcphdr;
	u8t i;
	u8t* iset;
...
err_t send_begin_of_epoch_msg(struct netif* netif)
{
	...
	*iset=Beginning_of_epoch;
	...
	pCount = (u16 *)iset;
	*pCount = TIM_GetCounter(TIM5);
//	pCount ++;
	//iset = (u8*) pCount;
//	printf("Time slot = %d count = %d\r\n", timeSlot, TIM_GetCounter(TIM5));
...
err_t send_follow(struct netif* netif, int count, int alias)
{
	struct ethbcp_hdr* bcphdr;
	u8t i;
	u8t* iset;	
	...
	if(time_slot_msg == NULL)
	{
		SSNP_DEBUG_PRINT("send_sync_time_msg():time_slot_msg is empty.\r\n");
		return ERR_MEMORY;
	}
	bcphdr=(struct ethbcp_hdr*)time_slot_msg->data;
...
err_t send_sync_time_msg(struct netif* netif)
{
	struct ethbcp_hdr* bcphdr;
	u8t i;
	u8t* iset;	
...
err_t send_follow_up_msg(struct netif* netif, int count)
{
	struct ethbcp_hdr* bcphdr;
	u8t i;
	u8t* iset;	
...
static unsigned int avertime_new(unsigned int arr[],u8t n)
{
	unsigned int sum;
	unsigned int max;
	unsigned int min;
	...
	m = 0;
	for(i=0;i<n;i++)
	{	if (arr[i] - aver > 300 || aver - arr[i]  < 300)
			continue;
...
unsigned int delayTime[100] = {0};
unsigned int tbimDelayTime = 0;
char sendPC[10];
u8t tbimAlias;
...
//*******************************************TBC特有代码*****************************************************************
	...
	{	// printf("recv reflect reply\r\n");
		...
		//	printf("T3 = %d  ", *pT);
			pT ++;
			T4 = TIM_GetCounter(TIM5);
	//		printf("T4 = %d  ", T4);
			minus1 = (T3 < T2) ? T3 - T2 : T3 - 0xffff - T2;
			minus2 = (T4 < T1) ? T1 - T4 : 0xffff - T4 + T1;
			delayTime[k ++] = (minus1 + minus2) / 2;
			
		//	printf("k = %d avg time = %d  \r\n", k, (minus1 + minus2) / 2);
	//	printf("k = %d avg time = %d  \r\n", k, delayTime[k - 1]);
			if (k == 100)     // DELAY_TIME_MEASURE_NUM  14
			{
				k = 0;
			//	tbimDelayTime = avertime_new(delayTime, 100);
			
			//	pT = (int *)&(sendPC[5]);
			//	*pT = sendPC[5] = '=';;
			//	sprintf(sendPC, "$%d*%d", tbimDelayTime,tbimAlias);
			//	printf("%s", sendPC);
