...
static void collectSample(char *arr, int index, char value) {
    int offset = index / 2;
	int fos = index % 2;
	if (fos == 0) {
	    arr[offset] |= (value & 0x0f);
	}
	else {
	    arr[offset] |= ((value << 4) & 0xf0);
	}
}
static int getSample(char *arr, int index) {
    char a = arr[0];
    int offset = index / 2;
	int fos = index % 2;
	if (fos == 0) {
	    return (int) (arr[offset] & 0x0f);
	}
	else {
	    return (int) ((arr[offset] >> 4) & 0x0f);
	}
}
static void uctsk_SSNP_TBIM_new(void* pdata)
{
				...
				/**/ 
				   ...
				   {    //printf("**************\r\n");
					...
					//	   timerValue[indexSamp] = syncTime();
						...
						sam_count ++;
						if (sam_count == SAMPLE_COUNT) {
						sam_count = 0;
				           readData = GPIO_ReadInputData(GPIOE);
	                       samp = (readData & 0x0f00) >> 8;
						   val = (char)(samp & 0x000f);
						   if (indexSamp >= START_SAMP)
						      collectSample(sampData, indexSamp - START_SAMP, val);
						   indexSamp ++ ;
					      // sampData[indexSamp++] = samp;
						  // printf("indexSamp = %d samp = %d\r\n", indexSamp, val);
						 // printf("i = %d samp = %d \r\n", indexSamp - 1, getSample(sampData, indexSamp - 1));
					/*     if (onoff) {
					...
					*/	   
				           if (indexSamp == START_SAMP + 20000)
				           {   
						   	   indexSamp = 0;
							   flagBeginEpoch = 0;
				               for (i = 0; i < 20000; i ++)
							       printf("i = %d samp = %d \r\n", i, getSample(sampData, i));
							 	  //printf("i = %d samp = %d \r\n", i, sampData[i]);
